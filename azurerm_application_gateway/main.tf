...
# Application Gateway resource - NO SSL certificate
resource "azurerm_application_gateway" "ag_as_waf_no_ssl" {
  count               = "${1 - var.configure_ssl}"
...
# Add diagnostic to AG - DO NOT configure SSL
resource "azurerm_monitor_diagnostic_setting" "ag_no_ssl" {
  count                      = "${1 - var.configure_ssl}"
  name                       = "${local.azurerm_application_gateway_diagnostic_name}"
  target_resource_id         = "${azurerm_application_gateway.ag_as_waf_no_ssl.id}"
  log_analytics_workspace_id = "${data.azurerm_log_analytics_workspace.workspace.id}"
